...
while ($game->getCurrentRound() < 10) {
    $game->newRound();

    foreach ($game->players as $player) {
        if ($player->isPatron()) {
            $player->collectAnte();
            print " round " . $game->getCurrentRound() . " Ante : " . $player->getName() . " tokens " . $player->getTokens() . " remaining\n";
        }
    }
    ...
    if ($game->getBonusWin()) {
        print " round " . $game->getCurrentRound() . " : Bonus win " . $game->dealer->hand[2]['glyph'] . " awarded at " . (($game->dealer->hole[2]['value'] ==1) ? 11 : $game->dealer->hole[2]['value']) . "X ante\n";
    } else {
    ...
    foreach ($game->players as $player) {
        if ($player->isPatron()) {
            print " round " . $game->getCurrentRound() . " : " . $player->getName() . " " . $player->handSummary() . " total: " . $player->handTotal() . " " . $player->outcome . ($player->getPayout() ? " payout : " . $player->getPayout() : "") . "\n";
        }
    }

    foreach ($game->players as $player) {
        $player->setHandHistory($player->handSummary() . " player total: " . $player->handTotal() . " dealer total: " . $game->dealer->handTotal() . " " . $player->outcome);
        if ($player->isPatron()) {
